...
const AvailableRidesPage = () => {
  ...
  const isNearRoutePath = (lat: number, lng: number, ride: Ride): boolean => {
    const minLat = Math.min(ride.pickup_lat, ride.drop_lat) - 0.15;
  ...
  return (
    <MainLayout>
      <div className="min-h-screen bg-background flex flex-col pb-20 lg:pb-8">
        <header className="sticky top-0 z-40 bg-background border-b border-border px-4 py-3 lg:px-6">
          <div className="flex items-center justify-between max-w-5xl mx-auto">
            <div className="flex items-center gap-3">
              <button onClick={() => navigate(-1)} className="p-1 lg:hidden">
                <ArrowLeft className="w-6 h-6 text-foreground" />
              </button>
              <h1 className="text-lg font-semibold text-foreground lg:text-xl">Available Rides</h1>
            </div>
                  ...
                  {filteredRides.map((ride, index) => (
                    <div key={ride.id}>
                      ...
                      <div
                        ...
                        className="bg-card border border-border rounded-xl p-4 shadow-sm cursor-pointer hover:shadow-md hover:border-foreground/20 transition-all"
                      >
